...
function render(){
 ...
 $('#staffCards').innerHTML=data.staff.map(s=>`<article class="staffCard"><div class="avatar"></div><h3>${esc(s.name)}</h3><p>${esc(s.specialty||'')}</p><button class="textBtn" data-book-staff="${s.id}">이 ${esc(data.store.staffLabel)}로 예약</button></article>`).join('');
 setupIntro();
}
...
function renderBooking(){
 ...
 const svc=data.services.find(s=>s.id===booking.serviceId); const eligible=data.staff.filter(st=>!svc||!st.services?.length||st.services.includes(svc.id));
...
let confirmCb=null;
function showConfirm({title,body,ok='확인',cancel='아니오',onOk}){$('#confirmTitle').textContent=title;$('#confirmBody').innerHTML=body;$('#confirmOk').textContent=ok;$('#confirmCancel').textContent=cancel;confirmCb=onOk;$('#confirmModal').classList.remove('hidden')}
