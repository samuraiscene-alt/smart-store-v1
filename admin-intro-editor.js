/* Smart Store - admin intro photo + text effect editor */
(() => {
  ...
  function mergeConfig(raw){
    ...
    if(raw && typeof raw==='object'){
      ...
      if(Array.isArray(raw.texts)){
        next.texts=next.texts.map((x,i)=>({...x,...(raw.texts[i]||{})}));
      }
    ...
    next.displaySeconds=clamp(next.displaySeconds,2,10)||4;
  ...
  function addStyles(){
    ...
    style.textContent=`
      ...
      .introEditorStatus{min-height:18px;color:#8e817b;font-size:12px;margin-top:8px}
  ...
  function setStatus(text=''){const el=q('#introEditorStatus');if(el)el.textContent=text;}
  function setSelected(key){selected=key;qa('[data-intro-select]').forEach(b=>b.classList.toggle('active',b.dataset.introSelect===key));qa('.introPanel').forEach(p=>p.classList.remove('active'));const panel=key==='image'?q('#introPanelImage'):key==='logo'?q('#introPanelLogo'):q(`#introPanelText${Number(key.replace('text',''))}`);panel?.classList.add('active');renderPreview();}

  function bindUi(){
    q('#introEditorSave').onclick=save;
    q('#introEditorSaveBottom').onclick=save;
    q('#introEditorReload').onclick=load;
    q('#introEditorOff').onclick=()=>setMode(false);
    q('#introEditorOn').onclick=()=>setMode(true);
    q('#introEditorPlay').onclick=playAnimations;
    qa('[data-intro-select]').forEach(b=>b.onclick=()=>setSelected(b.dataset.introSelect));
    q('#introEditorImageFile').onchange=previewImageFile;
    q('#introEditorLogoFile').onchange=previewLogoFile;
    q('#introFitContain').onclick=()=>{config.image={fit:'contain',x:0,y:0,zoom:1};syncControls();renderPreview();};
    q('#introFitCover').onclick=()=>{config.image={fit:'cover',x:0,y:0,zoom:1};syncControls();renderPreview();};
    q('#introZoomOut').onclick=()=>{config.image.zoom=clamp(config.image.zoom-0.1,0.5,4);syncControls();renderPreview();};
    q('#introZoomIn').onclick=()=>{config.image.zoom=clamp(config.image.zoom+0.1,0.5,4);syncControls();renderPreview();};
    q('#introImageReset').onclick=()=>{config.image.x=0;config.image.y=0;config.image.zoom=1;syncControls();renderPreview();};
    q('#introLogoEnabled').onchange=e=>{config.logo.enabled=e.target.checked;renderPreview();};
    q('#introLogoWidth').oninput=e=>{config.logo.width=Number(e.target.value);syncControls();renderPreview();};
    q('#introLogoReset').onclick=()=>{config.logo.x=50;config.logo.y=18;config.logo.width=34;syncControls();renderPreview();};
    q('#introDisplaySeconds').onchange=e=>config.displaySeconds=Number(e.target.value);
    q('#introShowSkip').onchange=e=>{config.showSkip=e.target.checked;renderPreview();};

    [0,1,2].forEach(i=>{
      q(`#introText${i}`).oninput=e=>{config.texts[i].text=e.target.value;renderPreview();};
      q(`#introTextColor${i}`).oninput=e=>{config.texts[i].color=e.target.value;renderPreview();};
      q(`#introTextSize${i}`).oninput=e=>{config.texts[i].size=Number(e.target.value);syncControls();renderPreview();};
      q(`#introTextAnimation${i}`).onchange=e=>config.texts[i].animation=e.target.value;
      q(`#introTextDelay${i}`).oninput=e=>{config.texts[i].delay=Number(e.target.value);syncControls();};
      q(`#introTextDuration${i}`).oninput=e=>{config.texts[i].duration=Number(e.target.value);syncControls();};
      q(`#introTextAlign${i}`).onclick=e=>{const b=e.target.closest('[data-align]');if(!b)return;config.texts[i].align=b.dataset.align;syncControls();renderPreview();};
      q(`#introTextReset${i}`).onclick=()=>{const defaults=DEFAULT_CONFIG.texts[i];config.texts[i].x=defaults.x;config.texts[i].y=defaults.y;syncControls();renderPreview();};
    });
    installGestures();
    setSelected('image');
  }
  ...
  async function load(){
    ...
    const {data:row,error}=await sb.from('stores').select('intro_mode,intro_media_url,intro_logo_url,intro_editor_config').eq('id',id).single();
    if(error){setStatus('불러오기 실패');console.error(error);return;}
    ...
    syncControls();renderPreview();setStatus('');
  }

  function setMode(enabled){q('#introEditorCard').dataset.enabled=enabled?'1':'0';syncControls();}
  ...
  function syncControls(){
    ...
    if(q('#introDisplaySeconds'))q('#introDisplaySeconds').value=String([3,4,5,6,8].includes(Number(config.displaySeconds))?config.displaySeconds:4);
  ...
  function renderPreview(){
    const preview=q('#'+PREVIEW_ID);if(!preview)return;
    preview.innerHTML='';
    const src=imageSource();
    if(src){
      const img=document.createElement('img');img.className='introPreviewImage';img.alt='';img.onload=()=>positionBackground(img,preview);img.src=src;preview.appendChild(img);if(img.complete)positionBackground(img,preview);
    }else{
      const empty=document.createElement('div');empty.className='introPreviewEmpty';empty.textContent='인트로 사진을 선택해주세요.';preview.appendChild(empty);
    }
    const lsrc=logoSource();
    if(config.logo.enabled && lsrc){
      const logo=document.createElement('img');logo.className='introPreviewLogo'+(selected==='logo'?' selected':'');logo.dataset.introOverlay='logo';logo.alt='';logo.src=lsrc;logo.style.left=`${config.logo.x}%`;logo.style.top=`${config.logo.y}%`;logo.style.width=`${config.logo.width}%`;preview.appendChild(logo);
    }
    config.texts.forEach((t,i)=>{
      if(!t.text)return;
      const el=document.createElement('div');el.className='introPreviewText'+(selected===`text${i}`?' selected':'');el.dataset.introOverlay=`text${i}`;el.textContent=t.text;el.style.left=`${t.x}%`;el.style.top=`${t.y}%`;el.style.fontSize=`${t.size}px`;el.style.color=t.color;el.style.textAlign=t.align;el.style.transform='translate(-50%,-50%)';preview.appendChild(el);
    });
    if(config.showSkip){const skip=document.createElement('div');skip.className='introPreviewSkip';skip.textContent='건너뛰기';preview.appendChild(skip);}
  }

  function positionBackground(img,preview){
    if(!img.naturalWidth||!img.naturalHeight)return;
    const rect=preview.getBoundingClientRect();if(!rect.width||!rect.height)return;
    ...
    const ox=(config.image.x/100)*rect.width,oy=(config.image.y/100)*rect.height;
  ...
  function playAnimations(){
    renderPreview();
    config.texts.forEach((t,i)=>{
      const el=q(`[data-intro-overlay="text${i}"]`);if(!el||t.animation==='none')return;
      ...
      el.style.animation='none';void el.offsetWidth;el.style.animation=`${names[t.animation]} ${t.duration}s ease ${t.delay}s both`;
    });
  }

  function distance(a,b){return Math.hypot(a.clientX-b.clientX,a.clientY-b.clientY);}
  function installGestures(){
    const preview=q('#'+PREVIEW_ID);if(!preview||preview.dataset.ready==='1')return;preview.dataset.ready='1';
    preview.addEventListener('touchstart',e=>{
      const overlay=e.target.closest?.('[data-intro-overlay]');
      if(overlay){
        const key=overlay.dataset.introOverlay;setSelected(key);
        const point=e.touches[0];const base=key==='logo'?config.logo:config.texts[Number(key.replace('text',''))];
        touchState={mode:'overlay',key,x:point.clientX,y:point.clientY,baseX:base.x,baseY:base.y};e.preventDefault();return;
      }
      setSelected('image');
      if(!imageSource())return;
      if(e.touches.length>=2)touchState={mode:'pinch',distance:distance(e.touches[0],e.touches[1]),zoom:config.image.zoom};
      else if(e.touches.length===1)touchState={mode:'image-drag',x:e.touches[0].clientX,y:e.touches[0].clientY,baseX:config.image.x,baseY:config.image.y};
      e.preventDefault();
    },{passive:false});
    preview.addEventListener('touchmove',e=>{
      if(!touchState)return;const rect=preview.getBoundingClientRect();
      if(touchState.mode==='overlay'&&e.touches.length){
        ...
        target.y=clamp(touchState.baseY+((e.touches[0].clientY-touchState.y)/rect.height)*100,0,100);renderPreview();e.preventDefault();return;
      }
      if(e.touches.length>=2){
        if(touchState.mode!=='pinch')touchState={mode:'pinch',distance:distance(e.touches[0],e.touches[1]),zoom:config.image.zoom};
        const d=distance(e.touches[0],e.touches[1]);if(touchState.distance>0)config.image.zoom=clamp(touchState.zoom*(d/touchState.distance),0.5,4);syncControls();renderPreview();e.preventDefault();return;
      }
      if(e.touches.length===1){
        ...
        config.image.y=clamp(touchState.baseY+((e.touches[0].clientY-touchState.y)/rect.height)*100,-150,150);renderPreview();e.preventDefault();
      }
    },{passive:false});
    const end=()=>{touchState=null;};preview.addEventListener('touchend',end);preview.addEventListener('touchcancel',end);window.addEventListener('resize',renderPreview);
  }

  function previewImageFile(){const f=q('#introEditorImageFile').files?.[0];if(!f)return;if(f.size>2*1024*1024){alert('인트로 사진은 2MB 이하로 사용해주세요.');q('#introEditorImageFile').value='';return;}if(!f.type.startsWith('image/')){alert('사진 파일만 사용할 수 있습니다.');return;}if(imageObjectUrl)URL.revokeObjectURL(imageObjectUrl);imageObjectUrl=URL.createObjectURL(f);config.image={fit:'contain',x:0,y:0,zoom:1};setMode(true);syncControls();renderPreview();}
  function previewLogoFile(){const f=q('#introEditorLogoFile').files?.[0];if(!f)return;if(f.size>2*1024*1024){alert('로고 이미지는 2MB 이하로 사용해주세요.');q('#introEditorLogoFile').value='';return;}if(!f.type.startsWith('image/')){alert('사진 파일만 사용할 수 있습니다.');return;}if(logoObjectUrl)URL.revokeObjectURL(logoObjectUrl);logoObjectUrl=URL.createObjectURL(f);config.logo.enabled=true;syncControls();renderPreview();}

  async function upload(file,prefix){
    const id=await waitForStore();if(!id)throw new Error('매장 연결을 확인해주세요.');
    if(file.size>2*1024*1024)throw new Error('이미지는 2MB 이하로 사용해주세요.');
    const ext=(file.name.split('.').pop()||'jpg').toLowerCase();const uid=crypto.randomUUID?.()||`${Date.now()}-${Math.random().toString(16).slice(2)}`;const path=`${id}/${prefix}-${uid}.${ext}`;
    const {error}=await sb.storage.from('store-media').upload(path,file,{upsert:false,contentType:file.type||undefined});if(error)throw error;return sb.storage.from('store-media').getPublicUrl(path).data.publicUrl;
  }
  function storagePath(url=''){const marker='/storage/v1/object/public/store-media/';const idx=String(url).indexOf(marker);return idx<0?'':decodeURIComponent(String(url).slice(idx+marker.length));}
  async function remove(url=''){const p=storagePath(url);if(p)await sb.storage.from('store-media').remove([p]);}

  async function save(){
    if(busy)return;const id=await waitForStore();if(!id){alert('매장 연결을 확인해주세요.');return;}
    const enabled=q('#introEditorCard').dataset.enabled==='1';const imageFile=q('#introEditorImageFile').files?.[0]||null;const logoFile=q('#introEditorLogoFile').files?.[0]||null;
    if(enabled&&!imageFile&&!imageUrl){alert('사진 인트로를 사용하려면 인트로 사진을 선택해주세요.');return;}
    busy=true;qa('#introEditorSave,#introEditorSaveBottom').forEach(b=>{b.disabled=true;b.textContent='저장 중...';});setStatus('저장 중...');
    try{
      let nextImage=imageUrl,nextLogo=logoUrl;
      if(imageFile){const old=nextImage;nextImage=await upload(imageFile,'intro');if(old&&old!==nextImage)remove(old).catch(()=>{});}
      if(logoFile){const old=nextLogo;nextLogo=await upload(logoFile,'intro-logo');if(old&&old!==nextLogo)remove(old).catch(()=>{});}
      const payload={intro_mode:enabled?'image':'none',intro_media_url:nextImage||'',intro_logo_url:nextLogo||'',intro_editor_config:config};
      const {error}=await sb.from('stores').update(payload).eq('id',id);if(error)throw error;
      imageUrl=nextImage;logoUrl=nextLogo;q('#introEditorImageFile').value='';q('#introEditorLogoFile').value='';
      if(imageObjectUrl){URL.revokeObjectURL(imageObjectUrl);imageObjectUrl='';}if(logoObjectUrl){URL.revokeObjectURL(logoObjectUrl);logoObjectUrl='';}
      setStatus('저장 완료 ✓');setTimeout(()=>setStatus(''),1600);renderPreview();
    }catch(err){alert(err?.message||'인트로 저장에 실패했습니다.');setStatus('저장 실패');}
    finally{busy=false;qa('#introEditorSave,#introEditorSaveBottom').forEach((b,i)=>{b.disabled=false;b.textContent=i===0?'저장':'저장';});}
  }

  async function install(){installUi();let tries=0;const timer=setInterval(()=>{installUi();tries++;if(document.getElementById(CARD_ID)&&typeof storeId!=='undefined'&&storeId){clearInterval(timer);load();}else if(tries>60)clearInterval(timer);},250);}
